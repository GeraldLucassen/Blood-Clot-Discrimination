...
for ii=1:size(spectrum_av,1)
    ...
    spectrum_av(wl<400)=NaN;                               % Values < 400 nm are approximtely zero and can be negative, so I just set them to NaN to avoid trouble down the line
    figure(44)
    plot(wl, spectrum_av(ii,:)./spectrum_av(1,:))
end
xlabel('Wavelength [nm]')
